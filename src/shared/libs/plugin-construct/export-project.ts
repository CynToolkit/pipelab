...
export const exportProjectAction = createAction({
  ...
  name: "Export folder",
  displayString:
    "`Export projet ${params.version ? `r${params.version}` : ''}`",
  meta: {},
  params: {
    folder: {
      label: "Folder",
...
const zipFolder = async (from: string, to: string) => {
  ...
  return new Promise<string>(async (resolve, reject) => {
    ...
    archive.on("progress", (progress: any) => {
      console.log(
        `Progress: ${progress.entries.processed} / ${progress.entries.total} files`
      );
    });
...
export const ExportProjectActionRunner = createActionRunner<
  typeof exportProjectAction
>(async (options) => {
  const { join } = await import("node:path");

  const outputPath = join(options.cwd, "c3_tmp_proj.c3p");

  const to = await zipFolder(options.inputs.folder, outputPath);
