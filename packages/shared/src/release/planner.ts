...
import type {
  ...
  ReleaseRegistry,
  ResolvedReleaseDestinationSlot,
  ValidationIssue,
} from "./types";
...
const sourceRef = (ref: ReleaseOutputRef): ref is { source: true } =>
  "source" in ref && ref.source === true;
const internalRef = (ref: ReleaseOutputRef): ArtifactRef =>
  sourceRef(ref) ? { source: true } : { producerId: ref.buildId, outputId: ref.targetId };
const keyFor = (ref: ReleaseOutputRef): string =>
  sourceRef(ref) ? "source" : `${ref.buildId}:${ref.targetId}`;
...
const accepted = (
  ...
  return evaluateArtifactAcceptance(
    artifact,
    definition.accepts,
    () => definition.acceptsWhen?.(artifact, { host, producer: definition }) ?? { accepted: true },
  );
};
...
const findAutomaticPath = (
  ...
  const visited = new Set<string>([
    candidate.descriptor.kind + JSON.stringify(candidate.descriptor),
  ]);
  while (queue.length) {
    const current = queue.shift()!;
    for (const transform of registry.producers.filter(
      (producer) => producer.planning.mode === "automatic",
    )) {
      ...
      for (const target of transform.targets) {
        ...
        const nextStep: AutomaticStep = {
          definition: transform,
          input: current.candidate,
          targetId: target.id,
          descriptor: nextDescriptor,
        };
        if (accepted(nextDescriptor, definition, host).accepted)
          return [...current.steps, nextStep];
        ...
        if (!visited.has(nextKey) && current.steps.length < registry.producers.length) {
          ...
          const nextCandidate: Candidate = {
            ref: { buildId: `__auto__${current.steps.length}`, targetId: target.id },
            artifactRef: { producerId: `__auto__${current.steps.length}`, outputId: target.id },
            descriptor: nextDescriptor,
          };
...
export const planRelease = (
  ...
  if (!source)
    issues.push(
      error(
        ...
        `Unknown source provider: ${config.source.provider}`,
        "source.provider",
      ),
    );
  else
    issues.push(
      ...source
        .validate(config.source.config)
        .map((issue) => ({ ...issue, path: issue.path ? `source.${issue.path}` : "source" })),
    );
  ...
  for (const [index, build] of (config.builds ?? []).entries()) {
    if (builds.has(build.id))
      issues.push(
        error(
          "release.build.id.duplicate",
          `Build ID is not unique: ${build.id}.`,
          `builds.${index}.id`,
        ),
      );
    ...
    configuredBuilds
      ...
      .flatMap((build) => {
        ...
        return build.targets
          ...
          .flatMap((target) => {
            const targetDefinition = producer.targets.find(
              (candidate) => candidate.id === target.id,
            );
            if (
              !targetDefinition?.output ||
              !accepted(targetDefinition.output, definition, context.host).accepted
            )
              return [];
            return [
              {
  ...
  const addAutomatic = (steps: AutomaticStep[], destinationBuildId: string): Candidate => {
    ...
    for (const step of steps) {
      ...
      graph.edges.push({
        from: sourceRef(candidate.ref) ? "source" : candidate.ref.buildId,
        to: id,
      });
      candidate = {
        ref: { buildId: id, targetId: step.targetId },
        artifactRef: { producerId: id, outputId: step.targetId },
  ...
  const resolveBuild = (buildId: string): BuildResolution | undefined => {
    ...
    if (resolving.has(buildId)) {
      issues.push(
        error(
          "release.build.input.cycle",
          `Build cycle detected at ${buildId}.`,
          buildPath(buildId, ".input"),
        ),
      );
      return undefined;
    }
    ...
    if (existing)
      return {
        ...
        producer: producers.find((item) => item.id === buildId)!,
      };
    const definition = registry.producers.find((candidate) => candidate.id === build.engine);
    if (!definition) {
      issues.push(
        error(
          "release.build.engine.unknown",
          `Unknown build engine: ${build.engine}.`,
          buildPath(buildId, ".engine"),
        ),
      );
      return undefined;
    }
    if (definition.planning.mode === "automatic")
      issues.push(
        error(
          "release.build.engine.automatic",
          `Automatic producer ${build.engine} cannot be selected as a build engine.`,
          buildPath(buildId, ".engine"),
        ),
      );
    ...
    if (build.input) {
      ...
      for (const upstream of configuredBuilds) {
        const directlyDependsOnCurrent =
          upstream.input && "buildId" in upstream.input && upstream.input.buildId === buildId;
        const hasStaticCompatibleOutput = potentialBuildOutputs(buildId, definition).some(
          (candidate) => "buildId" in candidate.ref && candidate.ref.buildId === upstream.id,
        );
        if (
          upstream.enabled &&
          upstream.id !== buildId &&
          (upstream.input || hasStaticCompatibleOutput) &&
          !directlyDependsOnCurrent &&
          !resolving.has(upstream.id) &&
          !resolved.has(`${upstream.id}:__profile__`)
        )
          resolveBuild(upstream.id);
      }
      candidates = [...resolved.values()].filter(
        (candidate) =>
    ...
    for (const candidate of candidates) {
      ...
      if (path) {
        if (selected) {
          issues.push(
            error(
              "release.build.input.ambiguous",
              `Build ${buildId} has multiple compatible inputs.`,
              buildPath(buildId, ".input"),
            ),
          );
          break;
        ...
        matchesArtifact(candidate.descriptor, definition.accepts) &&
        !directAcceptance.accepted &&
        "reason" in directAcceptance &&
        directAcceptance.reason
      ) {
        issues.push(
          error(
            "release.build.input.rejected",
            directAcceptance.reason,
            buildPath(buildId, ".input"),
          ),
        );
    ...
    if (!selected) {
      if (build.input)
        issues.push(
          error(
            "release.build.input.missing",
            `Build ${buildId} has no compatible input.`,
            buildPath(buildId, ".input"),
          ),
        );
    ...
    const targetConfigs = build.targets
      .filter((target) => target.enabled)
      .map((target) => {
        ...
        if (targetDefinition.buildType && targetDefinition.buildType !== build.type)
          issues.push(
            error(
    ...
    graph.edges.push({
      from: sourceRef(selected.ref) ? "source" : selected.ref.buildId,
      to: build.id,
    });
    ...
    for (const target of targetConfigs) {
      ...
      if (!descriptor) {
        issues.push(
          error(
            "release.build.target.descriptor",
            `Target ${target.id} does not declare an output descriptor.`,
      ...
      const candidate: Candidate = {
        ref: { buildId, targetId: target.id },
        artifactRef: { producerId: buildId, outputId: target.id },
        descriptor,
      };
    ...
    resolved.set(`${buildId}:__profile__`, {
      ref: { buildId, targetId: "__profile__" },
      artifactRef: { producerId: buildId, outputId: "__profile__" },
  ...
  function resolveRef(ref: ReleaseOutputRef, issuePath?: string): Candidate | undefined {
    ...
    if (!build || !build.enabled) {
      issues.push(
        error(
          "release.build.reference.disabled",
          `Referenced build is missing or disabled: ${ref.buildId}.`,
          issuePath,
        ),
      );
      return undefined;
    ...
    if (!target) {
      issues.push(
        error(
          "release.destination.input.invalid",
          `Referenced target is missing: ${ref.buildId}/${ref.targetId}.`,
          issuePath,
        ),
      );
      return undefined;
    ...
    if (!target.enabled) {
      issues.push(
        error(
          "release.destination.input.invalid",
          `Referenced target is disabled: ${ref.buildId}/${ref.targetId}.`,
          issuePath,
        ),
      );
      return undefined;
  ...
  for (const build of config.builds ?? [])
    if (build.enabled && !resolved.has(`${build.id}:__profile__`))
      issues.push(
        error(
          "release.build.input.missing",
          `Build ${build.id} has no compatible input.`,
          buildPath(build.id, ".input"),
        ),
      );
  ...
  for (const [index, destination] of (config.destinations ?? []).entries()) {
    ...
    if (!definition) {
      issues.push(
        error(
          "release.destination.unknown",
          `Unknown destination provider: ${destination.provider}.`,
          `destinations.${index}.provider`,
        ),
      );
      continue;
    }
    ...
    if (!destination.slots.some((slot) => slot.enabled))
      issues.push(
        error(
          ...
          `Destination ${destination.id} must have an enabled slot.`,
          `destinations.${index}.slots`,
        ),
      );
    const slots = destination.slots.flatMap<ResolvedReleaseDestinationSlot>((slot, slotIndex) => {
      if (!slot.enabled) return [];
      if (!slot.input) {
        issues.push(
          error(
            "release.destination.input.required",
            "Choose an output for this destination.",
            `destinations.${index}.slots.${slotIndex}.input`,
          ),
        );
        return [{ id: slot.id, enabled: slot.enabled, config: slot.config }];
      }
      ...
      const acceptance = evaluateArtifactAcceptance(
        candidate.descriptor,
        definition.accepts,
        () =>
          definition.acceptsWhen?.(candidate.descriptor, {
            host: context.host,
      ...
      if (!acceptance.accepted)
        issues.push(
          error(
            "release.destination.input.incompatible",
            "reason" in acceptance && acceptance.reason
              ? acceptance.reason
              : `Destination ${destination.id} cannot consume the selected output.`,
            `destinations.${index}.slots.${slotIndex}.input`,
          ),
        );
