...
const handlers = [
  mswHttp.post(
    "https://devs-api.poki.io/v1/games/:gameId/versions",
...
const bridge = http.createServer(async (req, res) => {
  try {
    ...
    console.log(
      `[MSW Bridge] Received ${req.method} ${originalProtocol}//${originalHost}${req.url}`,
    );
    ...
    console.log(
      `[MSW Bridge] MSW Response: ${mswResponse.status}, Body length: ${responseBody.length}`,
    );
...
test("End-to-End: Poki Upload Action via Pipelab CLI", async () => {
  ...
  console.log(
    `Running CLI via tsx: ${tsxBinary} ${cliSourcePath} run ${pipelineFile} --user-data ${userDataPath}`,
  );
