...
describe("workflow plugin task adapter", () => {
  it("adapts a plugin runner and declared artifact output", async () => {
    ...
    await task({ step: { id: "build", uses: "fake/build", artifacts: { output: { descriptor: { kind: "application" } } } }, inputs: {}, workspace: { root: "/tmp" }, filesystem: { ensureDirectory: async () => undefined }, processes: {} as never, logger: { info: vi.fn(), warn: vi.fn(), error: vi.fn() }, signal: new AbortController().signal, log: vi.fn(), logStream: vi.fn(), setArtifact } as never);
