...
const executionPlan = (workflow: Workflow) => workflow.steps.map((step) => ({
  ...
  startTime: 0,
  logs: [],
  ...(step.delivery ? { destinationId: step.delivery.destinationId, slotId: step.delivery.slotId } : {}),
}));

const historyArtifacts = (result: WorkflowResult): NonNullable<BuildHistoryEntry["artifacts"]> => result.artifacts.map((artifact, index) => {
  if (!("descriptor" in artifact)) return { id: `workflow-artifact-${index}`, name: artifact.name, path: artifact.path, size: 0, type: "file" as const };
  return { id: artifact.id, name: artifact.artifact, path: artifact.path, size: artifact.size ?? 0, type: "file" as const, descriptor: artifact.descriptor, version: artifact.version, stepId: artifact.stepId, artifact: artifact.artifact, checksum: artifact.checksum, cloud: artifact.cloud };
});

export const executeWorkflow = async (context: PipelabContext, configName: string, options: { release?: { version?: string; description?: string }; signal?: AbortSignal; onEvent?: (event: WorkflowEvent) => void; onRunCreated?: (id: string) => void | Promise<void> } = {}) => {
...
export const registerWorkflowHandlers = (context: PipelabContext, pluginsReady?: Promise<void>) => {
  ...
  handle("workflow:execute", async (_, { send, value }) => {
    ...
    try { await pluginsReady; const result = await executeWorkflow(context, value.name, { release: value.release, signal: controller.signal, onRunCreated: (id) => { runId = id; activeRuns.register(id, controller); return send({ type: "workflow-run", data: { runId: id } }); }, onEvent: (event) => void send({ type: "workflow-event", data: event }) }); await send({ type: "end", data: { type: "success", result } }); } catch (error) { logger().error("Release workflow failed:", error); await send({ type: "end", data: { type: "error", code: controller.signal.aborted ? "canceled" : "error", ipcError: error instanceof Error ? error.message : String(error) } }); } finally { if (runId) activeRuns.remove(runId, controller); }
  });
  handle("workflow:cancel", async (_, { send, value }) => await send({ type: "end", data: { type: "success", result: { result: activeRuns.cancel(value.runId) ? "ok" : "ko" } } }));
  handle("pipelab-cloud:artifact-download-url", async (_, { send, value }) => { try { await send({ type: "end", data: { type: "success", result: { url: await getPipelabCloudDownloadUrl(context, String(value.hostedArtifactId || "")) } } }); } catch (error) { await send({ type: "end", data: { type: "error", ipcError: error instanceof Error ? error.message : String(error) } }); } });
};
