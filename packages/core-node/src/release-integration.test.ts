...
describe("release provider integration wiring", () => {
  it("connects Construct web output to Electron and Steam", () => {
    const config: ReleaseConfig = { version: "3.0.0", id: "construct-release", project: "project", name: "Construct release", source: { provider: "@pipelab/plugin-construct/source", config: { path: "/game.c3p", profilePath: "/profile" } }, builds: [{ id: "electron", type: "desktop", engine: "@pipelab/plugin-electron/producer", enabled: true, config: {}, targets: [{ id: "windows-x64", enabled: true, config: {} }] }], destinations: [{ id: "steam", provider: "@pipelab/plugin-steam/destination", enabled: true, config: { accountConnectionId: "steam", appId: "123" }, slots: [{ id: "windows", enabled: true, input: { buildId: "electron", targetId: "windows-x64" }, config: { depotId: "456" } }] }] };
  ...
  it("connects Godot web output to Poki", () => {
    const config: ReleaseConfig = { version: "3.0.0", id: "godot-release", project: "project", name: "Godot release", source: { provider: "@pipelab/plugin-godot/source", config: { path: "/game" } }, builds: [{ id: "godot", type: "web", engine: "@pipelab/plugin-godot/producer", enabled: true, config: { executable: "godot" }, targets: [{ id: "web", enabled: true, config: { preset: "Web" } }] }], destinations: [{ id: "poki", provider: "@pipelab/plugin-poki/destination", enabled: true, config: { project: "game", name: "1.0", notes: "release" }, slots: [{ id: "web", enabled: true, input: { buildId: "godot", targetId: "web" }, config: {} }] }] };
  ...
  it("connects a Web folder source directly to Poki", () => {
    const config: ReleaseConfig = { version: "3.0.0", id: "web-direct", project: "project", name: "Web direct", source: { provider: "@pipelab/plugin-filesystem/web-folder-source", config: { path: "/web" } }, builds: [], destinations: [{ id: "poki", provider: "@pipelab/plugin-poki/destination", enabled: true, config: { project: "game", name: "1.0", notes: "release" }, slots: [{ id: "web", enabled: true, input: { source: true }, config: {} }] }] };
  ...
  it("keeps a generic ZIP source semantically generic", () => {
    const config: ReleaseConfig = { version: "3.0.0", id: "zip-direct", project: "project", name: "ZIP direct", source: { provider: "@pipelab/plugin-filesystem/zip-source", config: { path: "/game.zip" } }, builds: [], destinations: [{ id: "itch", provider: "@pipelab/plugin-itch/destination", enabled: true, config: { accountConnectionId: "itch", project: "owner/game" }, slots: [{ id: "zip", enabled: true, input: { source: true }, config: { channel: "html5" } }] }] };
