...
import { transformArtifactDescriptor, type CompiledArtifact, type ReleaseDestinationDefinition, type ReleaseProducerDefinition, type ReleaseSourceDefinition } from "@pipelab/shared";
...
const webFolderSource: ReleaseSourceDefinition = { ...folderSource, id: "@pipelab/plugin-filesystem/web-folder-source", label: "Web app folder", output: { kind: "application", platform: "web", container: "directory" }, compile: (config) => ({ steps: [{ id: "release-web-folder-source", uses: "@pipelab/plugin-filesystem/fs:copy", with: { from: config.path, to: "${{ variables.workspace }}/source" }, artifacts: { output: { descriptor: { kind: "application", platform: "web", container: "directory" } } } }], artifact: { reference: { stepId: "release-web-folder-source", artifact: "output" }, descriptor: { kind: "application", platform: "web", container: "directory" } } }) };
const zipSource = (id: string, label: string, output: ReleaseSourceDefinition["output"]): ReleaseSourceDefinition => ({
  ...
  compile: (config) => ({ steps: [{ id: `${id}-source`, uses: "@pipelab/plugin-filesystem/fs:copy", with: { from: config.path, to: "${{ variables.workspace }}/source.zip" }, artifacts: { output: { descriptor: output } } }], artifact: { reference: { stepId: `${id}-source`, artifact: "output" }, descriptor: output } }),
});
...
export const folderDestination: ReleaseDestinationDefinition = {
  ...
  compile: (artifact, destination, slot) => [{ id: `release-folder-${destination.id}-${slot.id}`, uses: "@pipelab/plugin-filesystem/fs:copy", needs: [artifact.reference.stepId], artifactInputs: { from: artifact.reference }, with: { to: destination.config.outputDir, recursive: true, overwrite: true, cleanup: true, ...slot.config }, delivery: { destinationId: destination.id, slotId: slot.id, artifact: artifact.reference } }],
};
...
export const zipDestination: ReleaseDestinationDefinition = {
  ...
  compile: (artifact, destination, slot) => [{ id: `release-zip-${destination.id}-${slot.id}`, uses: "@pipelab/plugin-filesystem/zip-v2-node", needs: [artifact.reference.stepId], artifactInputs: { folder: artifact.reference }, with: { outputPath: destination.config.outputPath, ...slot.config }, delivery: { destinationId: destination.id, slotId: slot.id, artifact: artifact.reference } }],
};
...
const unzipProducer: ReleaseProducerDefinition = {
  ...
  compile: (input: CompiledArtifact, config) => {
    const descriptor = transformArtifactDescriptor(input.descriptor, { container: "directory", format: undefined });
    const stepId = `${config.id}-output`;
    return { steps: [{ id: stepId, uses: "@pipelab/plugin-filesystem/unzip-file-node", needs: [input.reference.stepId], artifactInputs: { file: input.reference }, artifacts: { output: { descriptor } } }], artifacts: { output: { reference: { stepId, artifact: "output" }, descriptor } } };
  },
};
...
export default createNodeDefinition({
  ...
  release: { sources: [folderSource, webFolderSource, genericZipSource, webZipSource], producers: [passthroughProducer, unzipProducer], destinations: [folderDestination, zipDestination] },
});
