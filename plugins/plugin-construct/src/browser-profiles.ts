...
export const detectConstructAuthStatusFromResponse = (
  ...
  const body = payload as {
    ...
    response?: { userID?: unknown; token?: unknown };
  };
...
const waitForConstructAuthResponse = (page: import("playwright").Page) =>
  new Promise<BrowserProfileAuthStatus>((resolve) => {
    ...
    const onResponse = async (response: import("playwright").Response) => {
      ...
      const status = detectConstructAuthStatusFromResponse(
        response.url(),
        response.status(),
        payload,
      );
...
const newest = async (root: string) => {
  ...
  for (const path of paths) {
    try {
      value = Math.max(value, (await stat(path)).mtimeMs);
    } catch {
      /* disappearing browser files */
    }
  }
  return value || null;
};
...
const executable = (browser: string) => {
  ...
  const installedBrowser = names
    .map((name) => dirs.map((dir) => join(dir, name)).find(existsSync))
    .find(Boolean);
...
export const chromiumProfiles = async (root: string): Promise<ChromiumProfile[]> => {
  try {
    ...
    const profiles = Object.entries(state.profile?.info_cache || {}).map(([path, info]) => ({
      path,
      name: info.name || path,
    }));
    if (profiles.length) {
      return (
        await Promise.all(
          profiles.map(async (profile) => {
            ...
            return nestedProfiles.map((nested) => ({
              path: join(profile.path, nested.path),
              name: nested.name,
            }));
          }),
        )
    ...
    /* older or damaged browser state */
  }
  try {
...
export const inspectChromiumProfile = async (
  ...
  Pick<BrowserProfileCandidate, "addonCount" | "authStatus" | "lastUpdatedAt" | "usable">
> => {
  const inspection = await inspectChromiumProfileForBrowser(profile, "Chrome");
  return {
    ...inspection,
    lastUpdatedAt: await constructStorageUpdatedAt(profile),
    usable: inspection.addonCount !== null,
  };
};
...
const inspectChromiumProfileForBrowser = async (profile: string, browser: string) => {
  ...
  try {
    await access(join(profile, "Preferences"));
  } catch {
    return { addonCount: null, authStatus: "unknown" as const };
  }
  ...
  try {
    ...
    try {
      ...
      await page
        .goto("https://editor.construct.net/", { waitUntil: "domcontentloaded", timeout: 15000 })
        .catch(() => {});
      ...
      const addonCount = await page.evaluate(async () => {
        ...
        if (!db) return 0;
        const store = db.objectStoreNames.contains("keyvaluepairs")
          ? "keyvaluepairs"
  ...
  for (const [browser, root] of chromiumRoots()) {
    for (const profile of await chromiumProfiles(root)) {
      ...
      found.push({
        ...
        path,
        isDefault: profile.path.split(/[\\/]/).pop() === "Default",
        ...inspection,
        lastUpdatedAt: await constructStorageUpdatedAt(path),
        score: null,
        usable: inspection.addonCount !== null,
        reason:
          inspection.addonCount === null ? "Browser unavailable or profile is locked" : undefined,
      });
    }
  }
  return found;
...
const discoverFirefox = async (): Promise<BrowserProfileCandidate[]> => {
  const root =
    platform() === "win32"
      ...
      return [
        {
          ...
          profileName: name,
          path: normalize(full),
          isDefault: /^Default=1$/m.test(section),
          addonCount: null as number | null,
          authStatus: "unknown" as const,
          lastUpdatedAt: null as number | null,
          score: null as number | null,
          usable: false,
          reason: "Firefox profile discovery is read-only; Construct export uses Chromium",
        },
      ];
    });
  } catch {
...
export const discoverBrowserProfiles = async (): Promise<BrowserProfileCandidate[]> => {
  const candidates = [
    ...
    ...(await discoverFirefox()),
    ...(await discoverSafari()),
  ];
  ...
  for (const candidate of candidates) {
    ...
    candidate.score = Math.round(
      addonScore + Math.max(0, 1 - ageDays / 180) * 35 + (candidate.isDefault ? 5 : 0),
    );
  }
  return candidates.sort(
    (a, b) =>
      (b.score ?? -1) - (a.score ?? -1) ||
      (b.addonCount ?? -1) - (a.addonCount ?? -1) ||
