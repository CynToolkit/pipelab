...
import { savedFileMigrator, fileRepoMigrations, appSettingsMigrator, useLogger } from "@pipelab/shared";
...
export const registerMigrationHandlers = () => {
  ...
  handle("config:load", async (_, { send, value }) => {
    ...
    try {
      ...
      if (!migrator) {
        throw new Error(`No migrator found for configuration: ${name}. All files loaded via config:load MUST have a migration schema.`);
      }
